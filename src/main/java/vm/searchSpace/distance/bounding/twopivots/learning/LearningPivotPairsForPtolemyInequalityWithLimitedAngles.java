...
public class LearningPivotPairsForPtolemyInequalityWithLimitedAngles<T> {
    ...
    public void execute() throws InterruptedException {
        Map<Comparable, T> oMap = ToolsSpaceDomain.getSearchObjectsAsIdDataMap(searchSpace, sampleObjects);
        T[] sampleQueryArray = ToolsSpaceDomain.getData(sampleQueries.toArray(), searchSpace);
...
//        threadPool = Executors.newFixedThreadPool(1);
        ...
        if (RELATIVELY) {
            for (int p1Idx = 0; p1Idx < pivots.size() - 1; p1Idx++) {
                ...
                Comparable p1ID = searchSpace.getIDOfObject(p1);
                for (int p2Idx = p1Idx + 1; p2Idx < pivots.size(); p2Idx++) {
                    Object p2 = pivots.get(p2Idx);
                    Comparable p2ID = searchSpace.getIDOfObject(p2);
        ...
        Map<Comparable, Object> pMap = ToolsSpaceDomain.getSearchObjectsAsIdObjectMap(searchSpace, pivots);
        ...
        storage.storePivotPairs(name, searchSpace, retList);
    }
    ...
    private Map<Comparable, Integer> evaluateQuery(T qData, Map<Comparable, T> oMap, int[][] checkedPivotPairs) {
        ...
        T[] pivotsData = ToolsSpaceDomain.getData(pivots.toArray(), searchSpace);
        ...
        for (Map.Entry<Comparable, T> o : oMap.entrySet()) {
            ...
            for (int p1Idx = 0; p1Idx < pivots.size() - 1; p1Idx++) {
                Object p1 = pivots.get(p1Idx);
                T p1Data = searchSpace.getDataOfObject(p1);
                Comparable p1ID = searchSpace.getIDOfObject(p1);
                ...
                for (int p2Idx = p1Idx + 1; p2Idx < pivots.size(); p2Idx++) {
                    ...
                    Object p2 = pivots.get(p2Idx);
                    T p2Data = searchSpace.getDataOfObject(p2);
                    Comparable p2ID = searchSpace.getIDOfObject(p2);
