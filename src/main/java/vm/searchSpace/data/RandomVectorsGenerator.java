...
import vm.searchSpace.SimpleDatasetImpl;
...
public class RandomVectorsGenerator {
    ...
    private static final AbstractSearchSpacesStorage.OBJECT_TYPE[] OBJECT_TYPES = {AbstractSearchSpacesStorage.OBJECT_TYPE.DATASET_OBJECT, AbstractSearchSpacesStorage.OBJECT_TYPE.QUERY_OBJECT, AbstractSearchSpacesStorage.OBJECT_TYPE.PIVOT_OBJECT};
    private static final String[] OBJECT_TYPES_ID_PREFIXES = {"D", "Q", "P"};
    private int[] dimensions;
    private int[] sizesOfGeneratedSets;

    private final AbstractSearchSpacesStorage storage;

    public RandomVectorsGenerator(AbstractSearchSpacesStorage storage, int[] sizesOfGeneratedSets, int... dimensions) {
        this.storage = storage;
        this.dimensions = dimensions;
        this.sizesOfGeneratedSets = sizesOfGeneratedSets;
    }

    public RandomVectorsGenerator(AbstractSearchSpacesStorage storage) {
        this(storage, new int[]{1000000, 1000, 2560}, new int[]{10, 15, 25, 30, 35, 40, 50, 60, 70, 80, 90, 100});
    }
...
//                });
            ...
            ret[dimIdx] = new SimpleDatasetImpl(datasetName, querysetName, pivotsetName, storage);
    ...
    public List<Object> generateData(int sampleSize, int sampleDimension, String idPreffix, Random random, Function<Random, Float> generatingFunction) {
        ...
        AbstractSearchSpace searchSpace = storage.getSearchSpace();
        for (int i = 0; i < sampleSize; i++) {
            float[] data = generateRandomVector(sampleDimension, random, generatingFunction);
            String id = idPreffix + i;
            Object object = searchSpace.createSearchObject(id, data);
            ret.add(object);
        }
        return ret;
    }
