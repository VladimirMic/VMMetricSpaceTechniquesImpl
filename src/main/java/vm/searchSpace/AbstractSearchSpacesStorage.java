 ...
 * @param <T>
 */
public abstract class AbstractSearchSpacesStorage<T> {

    private static final Logger LOG = Logger.getLogger(AbstractSearchSpacesStorage.class.getName());
    protected final AbstractSearchSpace searchSpace;
    protected final SearchObjectDataToStringInterface<T> dataSerializator;
    ...
    public AbstractSearchSpacesStorage(AbstractSearchSpace<T> searchSpace, SearchObjectDataToStringInterface<T> dataSerializator) {
        this.searchSpace = searchSpace;
        this.dataSerializator = dataSerializator;
    }

    /**
     * Get iterator over search objects in a given dataset
     *
     * @param datasetName volutary, if it helps you to distinguish your datasets
     * @param params volutary, again, if you want to use them to get your search
     * objects
     * @return iterator over search objects in a specific dataset
     */
    ...
    public abstract void storeObjectToDataset(Object searchObject, String datasetName, Object... additionalParamsToStoreWithNewDataset);
    ...
    /**
     *
     * @param it iterator over search objects to storeObjectToDataset in a DB
     * @param count maximum number of search objects to storeObjectToDataset. 0
     * or -1 for all.
    ...
    public synchronized int storeObjectsToDataset(Iterator<Object> it, int count, String datasetName, Object... additionalParamsToStoreWithNewDataset) {
        ...
        for (counter = 0; it.hasNext() && counter <= maxCount; counter++) {
            ...
            if (counter % 10000 == 0) {
                LOG.log(Level.INFO, "Inserted {0} search objects", new Object[]{counter});
            }

        }
        LOG.log(Level.INFO, "Insert finished with {0} search objects inserted", new Object[]{counter});
        return counter;

    }

    public AbstractSearchSpace getSearchSpace() {
        return searchSpace;
    }
}
