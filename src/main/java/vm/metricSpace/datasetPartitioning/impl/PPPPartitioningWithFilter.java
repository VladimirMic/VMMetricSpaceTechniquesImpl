 ...
 * @author au734419
 */
public class PPPPartitioningWithFilter <T> extends AbstractDatasetPartitioning<T> {

    public static final Logger LOG = Logger.getLogger(PPPPartitioningWithFilter.class.getName());

    protected final DistanceFunctionInterface df;
    protected final List<Object> pivotsForPartitioning;
    protected final List<Object> pivotsForFilter;

    public PPPPartitioningWithFilter(AbstractMetricSpace<T> metricSpace, DistanceFunctionInterface<T> df, List<Object> pivotsForPartitioning, List<Object> pivotsForFilter) {
        super(metricSpace);
        this.df = df;
        this.pivotsForPartitioning = pivotsForPartitioning;
        this.pivotsForFilter = pivotsForFilter;
    }
...
//        int parallelism = 1;
        ...
        if (df instanceof CosineDistance) {
            lengthOfPivotVectors = ToolsMetricDomain.getVectorsLength(pivotsForPartitioning, metricSpace);
        }
    ...
    private class ProcessBatch extends AbstractDatasetPartitioning<T>.BatchProcessor {
        ...
        public void run() {
            ...
            float[] opDists = new float[pivotsForPartitioning.size()];
            for (int oCounter = 1; dataObjects.hasNext(); oCounter++) {
                ...
                for (int pCounterIdx = 0; pCounterIdx < pivotsForPartitioning.size(); pCounterIdx++) {
                    Object pivot = pivotsForPartitioning.get(pCounterIdx);
